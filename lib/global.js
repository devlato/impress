...
var initContext = function(context) {
  ...
  // Merge arrays
  //
  context.mergeArrays = function(/* arrays to merge */) {
    var i, j, array, result = [];
    for (i = 0; i < arguments.length; i++) {
      array = arguments[i];
      for (j = 0; j < array.length; j++) {
        if (result.indexOf(array[j]) === -1) result.push(array[j]);
      }
    }
    return result;
  }
  ...
  context.trim = function(s) {
    return s.replace(/^\s+|\s+$/g, '');
  };
    
  context.ltrim = function(s) {
    return s.replace(/^\s+/,'');
  };

  context.rtrim = function(s) {
    return s.replace(/\s+$/,'');
  };
  
  context.capitalize = function(s) {
